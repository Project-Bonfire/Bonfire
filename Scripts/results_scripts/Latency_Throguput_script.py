# Copyright (C) 2016 Siavoosh Payandeh Azad, Karl Janson
import os
...
for network_size in noc_size_list:
    ...
    for packet_size in packet_size_list:
        ...
        for PIR in pir_list:
            command = 'python ../../simulate.py -D '+str(network_size)+' -NI '+str(NI_depth)+' -Rand '+str(
                PIR)+' -PS '+str(packet_size)+" "+str(packet_size)+' -sim 50000 -end 62000 -lat'
            os.system(command)
            ...
            for line in f:
                if 'average' in line:
                    if 'throughput' in line:
                        throughput_result_file.write('%5s' % ("%.1f" % float(
                            float(line.split(":")[1].split(" ")[0])))+"    ")
                    else:
                        latency_result_file.write('%5s' % ("%.1f" % float(
                            float(line.split(":")[1].split(" ")[0])/clock_duration))+"    ")
        latency_result_file.write("\n")
        throughput_result_file.write("\n")
