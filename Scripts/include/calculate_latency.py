...
while line != '':
    ...
    if identifier in list(received_packets_dict.keys()):
        identifier = sender + "_" + receiver + \
            "_" + str(int(packet_id) + 16384)
...
while line != '':
    ...
    if identifier in list(sent_packets_dict.keys()):
        identifier = sender + "_" + receiver + \
            "_" + str(int(packet_id) + 16384)
...
if len(list(sent_packets_dict.keys())) != len(list(received_packets_dict.keys())):
    print("sent packets:", len(list(sent_packets_dict.keys())))
    print("received packets:", len(list(received_packets_dict.keys())))
    ...
    for identifier in sent_packets_dict:
        ...
        if packet_sent_time/1000.0 > warmup_time:
            if identifier in list(received_packets_dict.keys()):
                ...
                packet_length_list.append(
                    received_packets_length_dict[identifier])
            else:
                print(identifier)
    ...
    f.write("maximum packet latency:" +
            str(max(delay_list)/1000.0) + " ns" + "\n")

    f.write("minimmum packet latency:" +
            str(min(delay_list)/1000.0) + " ns" + "\n")
    ...
    print("average packet latency:", 
            "%.2f" % float(sum(delay_list)/(len(delay_list)*1000)), "ns!")

    bits_per_second = (sum(packet_length_list)*32) / \
        (float(max_recived_time-min_sent_time)/10**9)
