...
def find_events():
    ...
    for f in os.listdir(package.TRACE_DIR):
        if f.endswith('.txt'):
            ...
            if len(list(traffic_dic.keys())) > 0:
    ...
    while i < end_of_sim + 1:
        for item in list(dictionary_of_all.keys()):
            if i in list(dictionary_of_all[item].keys()):

                if i in list(time_dic.keys()):
                    time_dic[i].append(
                        [int(re.search(r'\d + ', item).group()), item[ - 5], dictionary_of_all[item][i]])
                else:
                    time_dic[i] = [
                        [int(re.search(r'\d + ', item).group()), item[ - 5], dictionary_of_all[item][i]]]
        i  += 0.5
    ...
    while i < end_of_sim + 1:
        if i in list(time_dic.keys()):
            for item in time_dic[i]:
                if item[2] in list(packet_dic.keys()):
...
def init():
    ...
    for item in range(0, noc_size ** 2):
        ...
        circle1 = plt.Circle((x, y), radius=0.1,
                             color='#8ABDFF', fill=False, lw=2)
        circle2 = plt.Circle((x - 0.2, y - 0.2), radius=0.05,
                             color='#8ABDFF', fill=False, lw=2)
        ...
        plt.gca().add_patch(patches.Arrow( - 0.4, (noc_size - 1)  + 
                                          0.2, 0.5, 0, width=0.03, color="black"))

        plt.gca().add_patch(patches.Arrow( - 0.4, (noc_size - 1)  + 
                                          0.2, 0,  - 0.5, width=0.03, color="black"))
    ...
    time_stamp_view = plt.text( - 0.35, (noc_size - 1)  + 
                               0.24, "time: 0 ns", fontsize=10)
    return None
...
def func(i):
    ...
    for event in list(x.keys()):
        if event not in list(packets.keys()):
    ...
    if time - 1 in list(events.keys()):
        del events[time - 1]
        print("removing all events of time:",
              time - 1, "events left:", len(events))

    packets_to_be_removed = []
    for packet in list(packets.keys()):
    ...
    time_stamp_view = plt.text( - 0.35, (noc_size - 1)  + 
                               0.24, "time: " + str(i/10.0) + " ns", fontsize=10)
    return packets,
...
def viz_traffic(network_size):
    ...
    print("generating the figure and axis for a "  + 
          str(noc_size) + " by " + str(noc_size) + " network!")
